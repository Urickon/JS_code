...
//GetElement - возвращает элемент, GetElements - возварщает NodeList из всех подходящих.
getElementsByID()
...
getElementById("myBtn").getAttribute("onclick")
...
Контекст вызова функции можно задавать при помощи:*/
bind()
let newContext = function.bind(object)
//Чтобы запустить функцию нужно еще раз вызвать newContext

function.call() //работает аналогично bind, но он сразу запускает функцию.

function.apply((this, [argsArray])// - работает как call, но аргументы передаются в массиве. 
...
members[...].define() //выдает информацию для любого элемента массив, тк метод общий.
