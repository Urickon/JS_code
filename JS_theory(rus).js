...
=================================================
Блоки могут переиспользоваться, меняться местами, перемещаться. Чтобы изменить внешний вид блока, структуру блока,
поведение блока, состояние блока (disabled) можно применять к нему модификаторы.
<div class="button button_disabled">...</div>
или даже несколько
<div class="button button_disabled button_themeSunny">...</div>

Для того, чтобы блоки можно было переиспользовать 

=================================================
Элементы - наполнение блоков, которые имеют смысл лишь внутри блока.

=================================================
Модификатор - добавочный стиль к элементу или блоку.
...
Можно разбивать на блоки. То есть элемент, вложенный в блок, сам будет блоком для вложенных в него. Это делается просто
добавлением к нему дополнительного класса:
<div class="header">
...
селекторов со специфического символа, например us-header, us-header-menu\

==================================================
Миксы - позволяют делать сам блок независимым и переиспользуемым, задавая стиль его позиционирования в модификаторе
Не только позиционирование, но и все другие переменные параметры имеет смысл выносить в модификатор, миксуя с основным.

<div class="menu">
	<div class="menu-bar bar menu_submit">  // так мы даем понять, что этот шаблон кнопки именно для подтвержения формы
		<span class="bar-text"></span>
	</div>
</div>

Также можно добавлять некоторые общие стилистические стили в миксы, например class="text", стилизующий все тексты.
...
React и Vue уже реализуют этот подход по дефолту, вью так вообще все вносит в один файл.
...
4. Нельзя делать смену имени элемента для добавления эффекта, для этого есть модификаторы
5. Делать блоки переиспользуемыми, выводя все переменные параметры в модификаторы (в БЭМ это называется миксы классов)
6. Элементы не должны существовать за пределами своих блоков
...
По сути данный принцип работает как принцип миксов в БЭМ. Фактически, мы выстраиваем иерархию классов стилей, как в ООП.
...
глобальную область выполнения - window.*/
