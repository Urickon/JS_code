...
выдавать один и тот же результат, а также не трогать внешнее окружение.
...
каррирования (карринга). Каррирование - это превращение функции с большим числом аргументов в последовательную
серию связных функций с одним аргументом. Предполагается, что они будут вызывать друг друга. Это может
пригодиться, напримр, при рассчете значений, когда одно число константное, мы не будем каждый раз
передавать его в аргумент, а сделаем вторую функцию и замнем ее на область видимости первой с этой константой.
Например:
function count (a, b) {
	return a * b
}
count(2, 3)
если b всегда один и тот же, то логичнее каррировать ее.
function count (b) {
	return function (a) {
		return a * b
	}
}
const multiplyThree = count(3)
multiplyThree(2) // 6
...
Большой пример реализации связного списка на Java Script*/
...
class LinkedList {
	...
	append (data) {
		// добавляет элемент в конец свзного списка
...
//null и undefined.
i++ //Сначала возвращает значение, потом инкрементирует(увеличивает)
++i //Сначала инкрементирует, потом возвращает
i-- //Сначала возвращает, потом декрементирует
--i //Сначала декремент, потом возвращает
...
Let живет только в скобках, в которых была определена. 
...
//Есть разновидность do while - ее смысл в том, что цикл выполняется как минимум один раз
do {
	//... your code
} while (true)
...
switch () {case ...}
/*Вообще это не цикл. Но... Это очень похожая внешне конструкция,
...
console.dir(obj) //вывод свойств объекта
...
console.table(array) //редставляет данные массива или объекта в таблице, удобной для восприятия.

>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>

window.history API

//Это встроенный объект в браузере, позволяет перемещаться по истории в пределах вкладки, обновить страницу
window.history.back() //назад
window.history.forward() //вперед
window.history.go() //более универсально, в аргументы передается число "шагов" назад или вперед
//если передать go(0), то вкладка перезагрузиться
window.history.length //узнать количество страниц в стеке истории

//=========================================================
window.history.pushState(data, title, [url]) //добавляет новый элемент в историю, не перезагружая при этом страницу
history.pushState({param: 'Value'}, '', 'myurl.html') //пример с пустым заголовком

window.history.replaceState(data, title, [url]) //работает схожим образом, но не добавляет элемент в историю, а заменяет текущий

//=========================================================
popstate //событие, срабатывающее при переходе от одного элемента истории к другому
...
Intl //встроенный объект для интернационализации данных
Intl.Collator //правильно сравнивает и сортирует строки
Intl.DataTimeFormat //форматирует дату и время в соответствии с языком
Intl.NumberFormat //форматирует числа схожим образом

//в объект передаются два основных аргумента - локаль и параметры
//локаль - это строка, содержащая в себе указание о языке, его варианте использования и стране.
//параметры устанавливают дополнительные аспекты

//пример:
const collator = new Intl.Collator()
console.log(collator.compare('ёж', 'яблоко')) //-1
...
//Наилучшим способом является performance.now() - возвращает TimeStamp с точностью до микросекунд. Пример:
const T = performance.now()
// your code
const T2 = performance.now()
const WORKING_TIME = T2 - T //ы миллисекундах (с плавающей запятой - до микросекунд)

>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>

"use strict"
/*Данная строка должна находиться в самом начале кода, иначе она не будет работать.
С помощью нее бразуер понимает, что данный код можно выполнять по стандарту ES5 (2009 год). 
Также, если в коде присутствуют классы или модули, то строгий режим включается автоматически.

Строгий режим не поддерживается очень старыми броузерами (IE9 и младше)

Строгий режим запрещает некоторые неоднозначные особенности JS, которые могут приводить к ошибкам.
Например,нельзя присвоить значение необъявленной переменной, нельзя изменять агрументы функции во время
ее выполнния через изменение arguments, больше слов зарезервировано для использования в будущем.*/

>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>

Lodash
require('lodash')
//Предоставляет удобныеметоды для работы с объектами и расширенным функционалом, а также удобной сокращенной формой записи.
//Основные методы:

_.map(), _.each(), _.remove(), _.reject(), _.find(), _.filter(), _.every(), _.reduce(), _.sortBy(), _.groupBy()

>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>=>

БЭМ
/*Методология бэм от Яндекса предполгает определенный стиль названия селекторов в css, а также
организации файлов и кода.
Есть много зарубежных аналогов, но в целом все их идеи сводятся к одному и тому же.

Базируется на блоках, элементах и модификаторах.

=================================================
Блоки могут переиспользоваться, меняться местами, перемещаться. Имя блока отвечает на вопрос "что это?"
Чтобы изменить внешний вид блока, структуру блока, поведение блока, состояние блока (disabled) можно применять к нему модификаторы.
<div class="button button__disabled">...</div>
или даже несколько
<div class="button button__disabled button__themeSunny">...</div>

Для того, чтобы блоки можно было переиспользовать, все общие характеристики передаются в отдельный селектор, к примеру button,
а все переменные выносятся в отдельные классы-модификаторы button__big, button__small (по описанию).

=================================================
Элементы - наполнение блоков, которые имеют смысл лишь внутри блока. Главное отличие блока и элемента в том, что элемент не самостоятелен.
Блок может переиспользоваться, а элемент всегда привязан к своему конкретному блоку.

=================================================
Модификатор - добавочный стиль к элементу или блоку. Имя модификтора отвечает на вопрос "какой?"

=================================================
Правила формирования названий селекторов
block-name__element_modificator или
blockName-element_modificator (React style)
либо
blockName--element__modificator

По сути, с бэм отпадает надобность в каскадности, так как у каждого тега будет свой класс.
Каскадность допускается для указания состояния:
...
==================================================
Не всегда стоит показывать в названии селектора полную вложенность, чтобы не получилось header-menu-navigation-item-bar-text
Это может обернуться задержкой при отрисовке стилей, когда после такого супер-друппер селектора идет, скажем span.

header-menu-navigation-item-bar-text > span

Как известно, броузер читает селекторы справа налево, так что он сначала возьмет все span на странице, а потом будет искать
среди них те, которые вложены в header-menu-navigation-item-bar-text. Это может снизить скорость отрисовки.

Предпочтительнее избегать вложенности элементов в названиях, оставляя лишь зависимость от блока, так мы не будем выстраивать бескончено
...
Еще важный момент - бывает так, что внутри header или другого крупного блока вставлены некие элементы, которые по смыслу
тоже выглядят как блоки. В этом случае можно отображать наследоване в имени класса уже от них, так как такие внутренние блоки
могут снова переиспользоваться.
<div class="header">
	<div class="header-menu">
		<div class="item">
...
В этом случае item это блок внутри блока, так как item будет использоваться по всей странице и не только в header.
Чтобы избежать длинной цепочки вложенности, можно наследовать только от item, делая все, что внутри item так же переиспользуемым.
Если же нужно добавить специфичности для item-text в header, то целесообразно просто добавить ему класс header-item-text, оставляя общий
item-text.
<div class="header">
	<div class="header-menu">
		<div class="item header-item">
			<div class="item-text header-item-text"></div>
		</div>
	</div>
</div>


Иногда, особенно при подключении сторонних библиотек (во избежание перезадания классов), имеет смысл начинать название всех
селекторов со специфического символа, например с-header, c-header-menu.

==================================================
Миксы - позволяют делать сам блок независимым и переиспользуемым, задавая стиль его общего для всей страницы оформления в
неких общих классах, типа button, а конкретные стили, отвечающие за расположение и специфическое оформление именно конкретной этой button
в классах типа menu-button, который будет оформлять все button в menu, но не другие. 
Если же, например, у нас в menu несколько одинаковых button, и класссы button и menu-button уже задействованы, то для выделения одной конкретной
button из них можно добавить ей класс (модификатор) типа menu__submit, который будет обозначать, что так оформляется только button в menu,
при этом отвечающие за отправку данных (submit), и никакие другие.

<div class="menu">
	<div class="menu-button button menu__submit">  // так мы даем понять, что этот шаблон кнопки именно для подтвержения формы
		<span class="button-text"></span>
	</div>
</div>

Также можно добавлять некоторые общие стилистические стили в миксы, например class="text", стилизующий все тексты на странице.

==================================================
Файловая структура по БЭМ

Можно хранить файлы стилей в методологии БЭМ - для каждого модификатора отдельный файл стилей.
Такой подход позволяет подключать только те модификаторы, которые нужны для реализации блока в определенный момент.

React и Vue уже реализуют этот подход по дефолту, Vue так вообще все вносит в один файл (разметку, скрипт и стили).

==================================================
Требования БЭМ

1. Не использовать инлайн-стили и холстеры(слишком много классов)
2. Не использовать !important
3. Соблюдать позицию в очереди подключения (как будет рендерится в таком порядке и записывать стили)
4. Нельзя делать смену имени элемента для добавления эффекта, для этого есть модификаторы (button__disabled)
5. Делать блоки переиспользуемыми, выводя все переменные параметры в классы типа header-button (только для кнопок в header)
6. Все общие характеристики элементов выносить в отдельные классы типа button
7. Элементы не должны существовать за пределами своих блоков, иначе это уже будут блоки
8. В идеале использовать селекторы только по классу (у каждого элемента должен быть свой класс)

==================================================
Есть другие системы типа БЭМ:

OOCSS - object oriented css. Разделяют классы на структурные (ширина, высота, расположение) и стилистические (цвет и пр.).
Например:
Глобально для всех кнопок создается класс .btn а потом добавляются классы для всех вариаций кнопок .btn-red .btn-big .btn-active
По сути данный принцип работает как принцип модификаторов в БЭМ. Фактически, мы выстраиваем иерархию классов стилей, как в ООП.
Но в данной системе не отражена структура стилей (не показана зависимость элементов друг от друга, нет разделения на блоки и элементы)

SMACSS - scalable and modular architecture for CSS. Основная цель - уменьшение количества кода. Весь код стилей разделяется на 5 групп:
1. base rules - стили основных элементов сайта (body, input, button, ul, ol). Используются в основнмо селекторы тебгов и аттрибутов.
2. layout rules - стили макета. Это стили глобальных элементов разметки шапки, футера, сайдбара. Как правило не повторяются более
одного раза на странице.
3. modules rules - стили модулей / блоков, которые могут использоваться много раз на странице.
4. state rules - стили состояния блоков и элементов. Единственный раздел, в котором допускается использовать !important. 
5. theme rules - файлы темы (ночь / день итп). Большинство сайтов не требуют.

Соответственно, нейминг происход с добавлением префиксов, обозначающих группу стилей, например:
layout-header (или l-header)
state-button*/
...
window.onerror //позволяет глоабльно отловить ошибку в коде
...
DOM нужен для того, чтобы мы через JS могли взаимодействовать с HTML. По сути, это единственное его
серьезное назначение. Он представляет узлы разметки в виде объектов, которые имеют свойства, и эти свойства 
можно изменять при помощи java script.*/
...
Жизненный цикл страницы(Life circle)

//Основные события жизненного цикла:
DOMContentLoaded //срабатывает на объекте document, соответственно, можно отловить через document.addEventListener("DOMContentLoaded", function())
//В этот момент готово DOM дерево.
//Ждет загрузки всех скриптов, но не стилей. Но если скрипт поместить после стиля, то придется ждать, пока он зарузится.
//У document есть свойсвто .readyState. У него три возможных значения, показывающих степень загрузки документа.
//Можно проверить, загрузился ли и либо сразу выполнить функцию, либо установить document.addEventListener("DOMContentLoaded")

window.onload //срабатывает, когда загрузились все ресурсы, включая стили и картинки.

window.onunload //когда посетитель покидает страницу, обычно отсылается статистика.

window.onbeforeunload //если пользователь собирается уйти со страницы или закрыть окно, то выполнится код.

/*Первым шагом броузера является создание DOM. По факту весь код приходит в байтах информации, браузер преобразует их в текст,
из текста создает узлы, из узлов создаются объекты, и только после этого все объекты компануются в дерево данных под названием DOM.

Чтобы корректно отобразить DOM нужно дождаться построения еще одного деорева данных, на этот раз стилей - CSSOM. Броузер не будет
показывать данные пока не прогрузит все возможные стили. После подгрузки оба дерева объединются в Render tree.*/

-------------------------------------

Метрики
/*Чтобы получить в JS коде размеры элемента есть специальные числовые метрики:*/
clientHeight / clientWidth //размеры элемента без border и полосы прокрутки, но с padding
scrollHeight / scrollWidth //размеры элемента с учетом прокрутки
offsetHeight / offfsetWidth //размеры включая border, но без прокрутки
...
Контекст выполнения
/*Контекст выполнения - то, где выполняется функция. На него ссылается this.

Стрелочные функции не имеют конткеста выполнения и arguments.
Они их заимствуют из функции, в которую вложены (если она есть), иначе будет ссылать на самую
глобальную область выполнения - window. Здесь стоит также отметить, что переменные, заданные через let и const не могут быть
видны в window, так можно получить лишь var.*/

Чистая функция
/*Функция может называться чистой при выполнении всего двух условий (pure function):
1. При одинковых входных параметрах функция всегда должна возвращать одинковый результат. Иначе можно
сказать, что передавая в нее определенный параметр, ты всегда знаешь, что она вернет.
Функция должна делать это в любое время выполнения программы (должна быть независима от внешнего окружения,
которое может меняться по ходу выполнения программы.)

2. Эта функция никак не должна взаимодействовать с остальной системой, она не должна: брать переменные
из внешней области видимости (т.к. они могут измениться и тогда функция начнет возвращать другое значение),
выводить что-либо в консоль, менять какой-либо объект. Говорят, что она не должна иметь побочных эффектов.

Плюсы чистой функции в том, что она уменьшает число ошибок (в теории) и упрощает тестирование.*/
...
try catch
/*Чтобы скрипт не падал в случае ошибки, неверного ввода или других неожиданностей, есть такая конструкция.*/
try {
...
//Работает эта конструкция лишь синхронно, а значит не отлавливает ошибки, например, в setTimeout().

Error
/*Объект Error, который передается в блок catch, имеет несколько важных свойств. Главные из них -
name, message, stack (отображает инфо из стека вызовов, где произоша ошибка).
По name мы сможем определить тип ошибки и обрабоать ее соответствующим образом.
Можно сделать и блок catch без переменной, в этом случае не будет доступа к ошибке.*/

//Нужно использовать этот блок как можно чаще, чтобы проработать все возможные ошибки, особенно
//при риске получения некорректных данных.

/*Часто бывает, что код выполняется корректно, но нет, скажем, нужного свойства объекта. И в этом случае
нужно тоже выбросить ошибку в блоке try, чтобы автоматически перейти в catch.
Делается это при помощи:*/
throw new Error('error_message')
//Можно генерировать разные виды ошибок
SyntaxError, ReferenceError, TypeError
//Из блока catch также можно выбросить ошибку дальше, во внешний блок, также с помощью throw

//Можно еще добавить в конце конструкцию finally, она будет выполнена в любом случае.

//Переменные внутри try..catch..finally локальны!

//Ошибку в глобальном коде можно обработать при помощи window.onerror
...
Но весь синхронный код, который написан внутри, к примеру, простой console.log будет запускаться при
создании промиса. То есть когда мы объявляем промис, он автоматически запускается и выполняет все,
что внутри (просто асинхронный код внутри будет выполнен с задержкой)
...
class Animal {
	...
	// также бывает статический метод, который можно вызывать только из самого класса, но не из его представителя.
...
Но есть ВАЖНЫЙ момент относительно этого метода. Он может мутировать (менять) свойства объекта.
Также можно поменять значение элемента по его ключу:
arr.forEach((item, index) => {
	arr[index]++
})

Это одна из особенностей JavaScript.*/
...
Array.every((currentValue, [index]) => {})
/*Метод позволяет проверить, удовлетворяют ли все элементы массива условию, заданному в передаваемой функции.
Иначе говоря, если функция возвратит true для каждого элемента в массиве, то и весь метод вернет true, иначе false.

Возвращает логиеское значение, не меняет сам массив, для котрого вызван.*/

---------------------

Array.unshift(element)
/*Вставляет метод в начало массива. Возвращает новую длину массива.*/

---------------------

Array.shift()
/*Метод удаляет первый элемент массива (с нулевым индексом) и возвращает его значение.*/
...
delete Obj.property
/*Удаляет свойство объекта. Также можно удалять элемент массива по индексу.*/
...
//Приблизительно также работает и клонирование массивов. На одноуровневом массиве работает spread,
//использование методов массиовов с пустыми аргументами (например, filter), метод Array.from(arr).
//Для глубокого клонирование используется либо JSON (с теми же проблемами), либо рекурсивное
