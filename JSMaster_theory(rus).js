...
/*
Data transfer object - это объект или интерфейс для передачи данных. Его цель - передать данные из
одной части программы в другую. В нем НЕ должно быть инкапсулировано никакой логики, максимум - геттеры и сеттеры.
Также в нем можно жестко детерминировать тип данных, которые должны передаваться.
*/

BO - business object
/*
В отличие от DTO реализует также и логику объекта. Он может самостоятельно изменять свои данные.
*/

VMC
/*
V -view - представление, не инкапсулирует логики, но отображает данные в удобном виде.
M - model (бизнес-модель, бизнес-логика) -модель данных. Не обязана знать о представлении ничего, обязана хранить данные в том формате,
в котором с ними удобно работать. У бизнеса нет задачи пердставить данные. Он должен хранить, обеспечивать согласованность,
обрабатывать, удалять, причем все это делать быстро.
С - contrtoller - преобразует данные между представлением и моделью. Обеспечивает их независимость друг от друга.
Контроллер можно разбивать на несколько слоев - скажем, один, который забирает данные из бизнса и передает дальше, а другие -
которые принимают данные от него и преобразуют для разных частей UI.
*/
...
ДЕБАГЕР CHROME DEVTOOLS

=====================================================================================================

JSDOC
/*Это стандартизированный способ покрытие кода комментариями и написания документации. Оперирует 
рядом ключевых слов, созданных для удобства ориентирования в коде. 
Различные IDE могут ориентироваться в JSDOC и показывать его при необходимости в любом месте кода,
где используется задокументированная функция. 

Внутри JsDoc можно свободно использовать html
Его главная задача - отображаться в браузере.
 ...
 * @returns {number} Сумма двух чисел, переданных на вход
 */

// Наиболее часто используемые ключевые слова:
@author // кто писал конкретный участок документации
@example // пример использования функции/объекта
@returns // возвращаемое значение функцией
@param // входные параметры функции
@this // значение this внутри функции
@throws // отбрасывает отключение
...
/*Условно существует две модели: OSI ()
Сетевая модель OSI — сетевая модель стека сетевых протоколов OSI. 
Посредством данной модели различные сетевые устройства могут взаимодействовать друг с другом. 
Модель определяет различные уровни взаимодействия систем. Принято различать семь таких уровней.
